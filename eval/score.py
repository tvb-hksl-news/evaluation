...
def bleu(references, hypotheses, level="word"):
    ...
    bleu_scores = metrics.sacrebleu.raw_corpus_bleu(
...
def get_alignment(r, h, d):
    ...
    return (
        {
            "align_ref_lst": align_ref_lst[::-1],
            "align_hyp_lst": align_hyp_lst[::-1],
            "align_lst": alignlist[::-1],
        },
...
def score_slr(hyp, ref) -> dict:
    hyp = [clean_tvb(h) for h in hyp]
    ref = [clean_tvb(r) for r in ref]
    scores = wer_list_per_sen(ref, hyp)
    return scores


def score_slt(hyp, ref):
    scores = {}
    scores |= bleu(ref, hyp, level="char")
    scores["rouge"] = rouge(ref, hyp, level="char")
    return scores


def main():
    parser = argparse.ArgumentParser(
        description="This is the official scoring script for the TVB-HKSL-News dataset. "
        "It scores a hypothesis file against a reference file using either SLR or SLT metrics."
    )
    parser.add_argument(
        "hyp",
        type=Path,
        help="Path to the hypothesis file, containing one sentence per line.",
    )
    parser.add_argument(
        "--split",
        choices=["dev", "test"],
        default="dev",
        help="Dataset split to be used: 'dev' or 'test'.",
    )
    parser.add_argument(
        "--task",
        choices=["slr", "slt"],
        default="slr",
        help="Scoring task to be used: 'slr' for Sign Language Recognition or 'slt' for Sign Language Translation.",
    )
    parser.add_argument(
        "--signer1-only",
        action="store_true",
        help="Only score signer 1",
    )
    ...
    if args.split == "test":
        df = pd.read_csv(split_dir / "test.csv", sep="|")
    elif args.split == "dev":
        df = pd.read_csv(split_dir / "dev.csv", sep="|")
    else:
        raise ValueError("Unknown split")

    df = df[df["signer"] == "Signer1"] if args.signer1_only else df

    df = df.sort_values("id")

    if len(df) != len(hyp):
        raise ValueError(
            f"Number of sentences in the hypothesis file ({len(hyp)}) does not "
            f"match the number of sentences in the {args.split} split ({len(df)})."
        )

    if args.task.lower() == "slr":
        ref = df["glosses"].str.strip().tolist()

        # Postprocess for SLR
        ref = [postprocess(r) for r in ref]
        hyp = [postprocess(h) for h in hyp]
    elif args.task.lower() == "slt":
        ref = df["words"].str.strip().tolist()
    else:
        raise ValueError(f"Unknown scoring task {args.task}")

    if args.task.lower() == "slr":
        scores = score_slr(hyp=hyp, ref=ref)
    elif args.task.lower() == "slt":
        scores = score_slt(hyp=hyp, ref=ref)
    else:
        raise ValueError(f"Unknown scoring task {args.task}")
